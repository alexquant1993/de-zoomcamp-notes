...
```
In Windows, the export command is not available. Instead, you can just paste the full json path to the following command in bash. Or you can use set command in cmd: `set GOOGLE_APPLICATION_CREDENTIALS=C:\Users\username\.gc\ny-rides.json`. In order to reference this env variable in bash, you need to use %GOOGLE_APPLICATION_CREDENTIALS%. For example, `gcloud auth activate-service-account --key-file %GOOGLE_APPLICATION_CREDENTIALS%`
