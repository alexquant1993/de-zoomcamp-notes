...
Make sure you don't stop this container. Open a new terminal, if necessary.
```bash
docker run -it \
  -e POSTGRES_USER="root" \
  -e POSTGRES_PASSWORD="root" \
...
- First install chocolotey (https://chocolatey.org/install)
- Then install terraform (https://learn.hashicorp.com/tutorials/terraform/install-cli). choco install terraform
